...
######################################
### GSDIII-AGL_KO vs WT - PCi1426  ###
######################################


coldata$cell_line <- as.character(coldata$cell_line)
coldata[c(22:24),"cell_line"]  <- c(rep("PCi1426_WT", 3 ))
coldata$cell_line <- as.factor(coldata$cell_line)


cdition1 <- "AGL_KO"
cdition2Ctrl <- "PCi1426_WT"
contrastO <- "cell_line"
...
###############################################
### GSDIII-AGL_KO vs WT - PCi1426  ( - B2 ) ###
###############################################
...
coldata$cell_line <- as.character(coldata$cell_line)
coldata[c(23:24),"cell_line"]  <- c(rep("PCi1426_WT", 2 ))
coldata$cell_line <- as.factor(coldata$cell_line)
...
rownames(annotation_col) <- rownames(coldata)
mat_colors <- list(
  cell_line = brewer.pal(length(unique(coldata$cell_line)), "Paired"))
names(mat_colors$cell_line) <- unique(coldata$cell_line)
...
write.csv2(finalDE, paste0("../results/NGS82_", cdition1, "-vs-", cdition2Ctrl,"_log2FC0-4_withoutB2.csv"))
finalDE_padj05 <- finalDE[which(finalDE$padj < 0.05 ),]
finalDE_padj05_BM20 <- finalDE_padj05[which(finalDE_padj05$baseMean >= 20 ),]
write.csv2(finalDE_padj05_BM20, paste0("../results/NGS82_", cdition1, "-vs-", cdition2Ctrl,"_log2FC0-4_padj0-5_BM20_withoutB2.csv"))
